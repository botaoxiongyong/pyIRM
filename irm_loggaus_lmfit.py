...
def draw_fig():
    plsq, x_fit, y_gradient = leasqr_fit()
    ...
    ax.scatter(x_measure, np.gradient(y_measure), facecolors='white', edgecolors='k', s=15, marker='o', alpha=1, label='Measured')
    #ax.plot(10**x_fit, y_gradient)
    #ax.plot(10**x_fit, y_fit, color='green')

    y_components = func_least(x_fit, plsq)
